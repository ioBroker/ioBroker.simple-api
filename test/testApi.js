...
describe('Test RESTful API', function () {
    ...
    it('Test RESTful API: setBulk - must set values', done => {
        request(
            `http://127.0.0.1:18183/setBulk?${TEST_STATE_ID}=50&system.adapter.simple-api.0.alive=false`,
            (error, response, body) => {
    ...
    it('Test RESTful API: setBulk(POST) - must set values', done => {
        request(
            ...
            (error, response, body) => {
                console.log(
                    `setBulk/?${TEST_STATE_ID}=50&system.adapter.simple-api.0.alive=false&javascript.0.test-string=bla%26fasel%2efoo%3Dhummer+hey => ${JSON.stringify(body)}`,
                );
                expect(error).to.be.not.ok;
                ...
                request(
                    ...
                    (error, response, body) => {
                        console.log(
                            `getBulk/${TEST_STATE_ID},system.adapter.simple-api.0.alive,javascript.0.test-string => ${body}`,
                        );
                        expect(error).to.be.not.ok;
                        const obj = JSON.parse(body);
                        expect(obj[0].val).equal(50);
