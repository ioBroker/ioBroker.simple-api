...
function SimpleAPI(server, webSettings, adapter, instanceSettings, app) {
    ...
    this.objectChange = function (id, _obj) {
        // Clear from cache, will be reinitialized on next usage
        if (that.cachedIds.has(id)) {
            const name = that.cachedIds.get(id).originId;
            that.cachedIds.delete(id);
            that.cachedNames.delete(name);
        }
    };

    function restApiPost(req, res, command, oId, values, ack) {
        let status = 500;
            ...
            switch (command) {
                case 'setBulk':
                    ...
                    for (const _id in values) {
                        ...
                        const resIndex = cnt;
                        cnt++;
                        that.adapter.log.debug(`${resIndex}: "${_id}"`);
                        ...
                        findState(_id, values.user, (err, id, originId) => {
                            if (err) {
                                ...
                                response[resIndex] = {error: `datapoint "${_id}" not found`};
                                if (!--cnt && !done) {
                                    done = true;
                                    doResponse(res, responseType, status, headers, response, values.prettyPrint);
                                }
                            } else {
                                const usedId = (_id === originId ? originId : id);
                                ...
                                adapter.setForeignState(id, values[usedId], ack, {
                                    ...
                                    if (err) {
                                        ...
                                        response[resIndex] = {id: id, val: values[usedId]};
                ...
                case 'setValueFromBody': {
                    ...
                    for (let k = 0; k < oId.length; k++) {
                        const resIndex = k;
                        that.adapter.log.debug(`${resIndex}: "${oId[k]}"`);
                        ...
                        findState(oId[k], values.user, (err, id, originId) => {
                            if (err) {
                                ...
                                response[resIndex] = {error: `datapoint "${originId}" not found`};
                                if (!--cnt && !_done) {
                                    _done = true;
                                    doResponse(res, responseType, status, headers, response, values.prettyPrint);
                                }
                            } else {
                                const usedId = (oId[k] === originId ? originId : id);
                                that.adapter.log.debug(`POST-${command} for id=${id}, oid=${originId}, used=${usedId}, value=${values[usedId]}`);
                                if (values[usedId] === 'true') {
                                    values[usedId] = true;
                                } else if (values[usedId] === 'false') {
    ...
    function findState(idOrName, user, type, callback) {
        ...
        if (that.cachedIds.has(idOrName)) {
            const data = that.cachedIds.get(idOrName);
            return setImmediate(() => callback(null, data.id, data.originId));
        } else if (that.cachedNames.has(idOrName)) {
            const data = that.cachedNames.get(idOrName);
            return void setImmediate(() => callback(null, data.id, data.originId));
        }

        adapter.findForeignObject(idOrName, type, {user: user, checked: true}, (err, id, originId) => {
            if (originId && typeof originId === 'object' && originId.en) {
                originId = originId.en;
            }
            that.cachedIds.set(id, { id, originId});
            if (idOrName !== id) {  // search was for a name, so also cache the name
                that.cachedNames.set(originId, { id, originId});
            }
            callback(err, id, originId);
        });
    }
    ...
    this.restApi = async function (req, res, isAuth, isChecked) {
        ...
        if (req.method === 'POST') {
            restApiPost(req, res, command, oId, values, ack);
            return;
        }
                ...
                for (let g = 0; g < oId.length; g++) {
                    ...
                    getState(oId[g], values.user, (err, obj, id, originId) => {
                        if (err) {
                            ...
                            pcnt = 0;
                            !pErrSent && doResponse(res, responseType, status, headers, {error: err}, values.prettyPrint);
                            pErrSent = true;
                            return;
                        } else if ((!id && originId) || obj === undefined) {
                            response[resIndex] = `error: datapoint "${oId[g]}" not found`;
                        } else {
                            let val = obj.val;
                            if (values.json) {
                                try {
                            ...
                            response[resIndex] = val;
                            status = 200;
                        }

                        !--pcnt && !pErrSent && doResponse(res, (status === 500 ? 'plain' : responseType), status, headers, response.join('\n'), values.prettyPrint);
                    });
                }
                break;
                ...
                for (let k = 0; k < oId.length; k++) {
                    ...
                    getState(oId[k], values.user, (err, state, id, originId) => {
                        ...
                        if (err) {
                            ...
                            !gErrSent && doResponse(res, responseType, status, headers, {error: err}, values.prettyPrint);
                            gErrSent = true;
                        } else if (!id && originId) {
                            response[resIndex] = {error: `datapoint "${originId}" not found`};
                            !--gCnt && !gErrSent && doResponse(res, responseType, status, headers, response.length === 1 ? response[0] : response, values.prettyPrint);
                        } else {
                            const vObj = state || {};
                            status = 200;
                            ...
                            that.adapter.getForeignObject(id, (err, obj) => {
                                if (obj) {
                                    for (const attr in obj) {
                                        if (obj.hasOwnProperty(attr) && vObj[attr] === undefined) {
                                            vObj[attr] = obj[attr];
                                        }
                                    }
                                }

                                response[resIndex] = vObj;
                                !--gCnt && !gErrSent && doResponse(res, responseType, status, headers, response.length === 1 ? response[0] : response, values.prettyPrint);
                            });
                        }
                    });
                ...
                for (let b = 0; b < oId.length; b++) {
                    ...
                    getState(oId[b], values.user, (err, state, id, originId) => {
                        if (err) {
                            ...
                            if (id) status = 200;
                            state = state || {};
                            response[resIndex] = {id: id, val: state.val, ts: state.ts, ack: state.ack};
                            if (!--bcnt && !_done) {
                                _done = true;
                                doResponse(res, responseType, status, headers, response, values.prettyPrint);
                            }
                        ...
                        if (err) {
                            ...
                            adapter.setForeignState(id, values.val, ack, {
                                ...
                                if (err) {
                                    ...
                                    if (wait && that.restApiDelayed.id === id && that.restApiDelayed.timer) {
                                        clearTimeout(that.restApiDelayed.timer);
                                        that.restApiDelayed.timer = null;
                ...
                for (const _id in values) {
                    ...
                    findState(_id, values.user, (err, id, originId) => {
                        ...
                        if (err) {
                            ...
                            response[resIndex] = {error: `error: datapoint "${originId}" not found`};
                            if (!--cnt && !done) {
                                done = true;
                                doResponse(res, responseType, status, headers, response, values.prettyPrint);
                            }
                            ...
                            adapter.setForeignState(id, values[usedId], ack, {
                                ...
                                if (err) {
                                    ...
                                    response[resIndex] = {id: id, val: values[usedId], value: values[usedId]};
                                    if (!--cnt && !done) {
                                        done = true;
                                        status = 200;
                                        doResponse(res, responseType, status, headers, response, values.prettyPrint);
