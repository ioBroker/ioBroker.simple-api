                    ...
                    for (var _id in values) {
                        ...
                        findState(_id, values.user, function (err, id, originId) {
                            ...
                            if (!id) {
                                ...
                                if (values[originId] === 'true') {
                                    values[originId] = true;
                                } else if (values[originId] === 'false') {
                                    values[originId] = false;
                                } else if (!isNaN(values[originId]) && values[originId] == parseFloat(values[originId])) {
                                    values[originId] = parseFloat(values[originId]);
                                }
                                ...
                                adapter.setForeignState(id, values[originId], false, {user: values.user}, function (err, id) {
                                    if (err) {
                                        ...
                                        status = 200;
                                        adapter.log.debug('Add to Response: ' + JSON.stringify({id:  id, val: values[originId]}));
        ...
        switch (command) {
                ...
                if (values.value === undefined && values.val === undefined) {
                    ...
                    findState(oId[0], values.user, function (err, id, originId) {
                        ...
                        if (id) {
                            ...
                            adapter.setForeignState(id, values.val, false, {user: values.user}, function (err) {
                                if (err) {
                                    doResponse(res, 'plain', 500, headers, 'error: ' + err, values.prettyPrint);
                                    wait = 0;
                                } else
            ...
            case 'setBulk':
                ...
                for (var _id in values) {
                    ...
                    cnt++;
                    findState(_id, values.user, function (err, id, originId) {
                        adapter.log.debug('ID=' + id + ' / originId=' + originId);
                        if (err) {
                            ...
                            adapter.setForeignState(id, values[originId], false, {user: values.user}, function (err, id) {
                                if (err) {
                                    ...
                                    adapter.log.debug('Add to Response-Get: ' + JSON.stringify({id: id, val: values[originId], value: values[originId]}));
                                    response.push({id: id, val: values[originId], value: values[originId]});
                                    if (!--cnt) doResponse(res, responseType, status, headers, response, values.prettyPrint);
                                }
                            });
                        }
            ...
            case 'getObjects':
            case 'objects':
                adapter.getForeignObjects(values.pattern || parts[2] || '*', values.type, {user: values.user}, function (err, list) {
                    if (err) {
                        status = 500;
