            ...
            switch (command) {
                ...
                case 'setValueFromBody':
                    ...
                    for (var j = 0; j < arr.length; j++) {
                        arr[j] = arr[j].split('=');
                        values[arr[j][0].trim()] = (arr[j][1] === undefined) ? body : arr[j][1];
                    }
                    ...
                    if (!idTohandle) {
                        doResponse(res, 'plain', 500, headers, 'error: no datapoint to set found', values.prettyPrint);
                    }
                    else {
                        that.adapter.log.debug('POST-' + command + ': values = ' + JSON.stringify(values));
                        that.adapter.log.debug('POST-' + command + ': idTohandle="' + idTohandle + '"');
                        findState(idTohandle, values.user, function (err, id, originId) {
                            if (err) {
                                doResponse(res, 'plain', 500, headers, 'error: ' + err, values.prettyPrint);
                            } else if (!id) {
                                doResponse(res, 'plain', 500, headers, 'error: datapoint "' + originId + '" not found', values.prettyPrint);
                            } else {
                                var usedId = (values[originId]?originId:id);
                                that.adapter.log.debug('POST-' + command + ' for id=' + id + ', oid=' + originId + ', used=' + usedId + ', value='+values[usedId]);
                                if (values[usedId] === 'true') {
                                ...
                                adapter.setForeignState(id, values[usedId], false, {user: values.user}, function (err, id) {
                                    if (err) {
                                        ...
                                        adapter.log.debug('Response: ' + JSON.stringify({id:  id, val: values[usedId]}));
                                        doResponse(res, responseType, 200, headers, [{id:  id, val: values[usedId]}], values.prettyPrint);
                                    }
                                });
                            }
                        });
