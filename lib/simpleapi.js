...
function SimpleAPI(server, webSettings, adapter, instanceSettings, app) {
    ...
    this.objectChange = function (id, _obj) {
        ...
        if (this.cachedIds.has(id)) {
            const name = this.cachedIds.get(id).originId;
    ...
    function findState(idOrName, user, type, callback) {
        ...
        if (this.cachedIds.has(idOrName)) {
            const data = this.cachedIds.get(idOrName);
            return void callback(null, data.id, data.originId);
        } else if (this.cachedNames.has(idOrName)) {
            const data = this.cachedNames.get(idOrName);
            return void callback(null, data.id, data.originId);
        }
        ...
        adapter.findForeignObject(idOrName, type, {user: user, checked: true}, (err, id, originId) => {
            ...
            this.cachedIds.set(id, { id, originId});
            if (idOrName !== id) {  // search was for a name, so also cache the name
                this.cachedNames.set(originId, { id, originId});
            }
