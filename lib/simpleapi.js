                    ...
                    for (const _id in values) {
                        ...
                        findState(_id, values.user, (err, id, originId) => {
                            if (err) {
                                err = err.toString();
                                status = 500;
                                if (err.indexOf(ERROR_PERMISSION) !== -1) {
                                    status = 401;
                                }
                                if (!done) {
                                    done = true;
            ...
            if (!isAuth) {
                ...
                that.checkPermissions(values.user, command, err => {
                    if (!err) {
                        ...
                        doResponse(res, responseType, 401, headers, {error: err.toString()}, values.prettyPrint);
                    }
                });
                return;
            }
                    ...
                    findState(oId[0], values.user, (err, id, originId) => {
                        if (err) {
                            ...
                            adapter.setForeignState(id, values.val, ack, {
                                ...
                                if (err) {
                                    status = 500;
                                    if (err.toString().indexOf(ERROR_PERMISSION) !== -1) {
                                        status = 401;
                                    }
                                    doResponse(res, responseType, status, headers, {error: err}, values.prettyPrint);
                                    wait = 0;
