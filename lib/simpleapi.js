...
function SimpleAPI(server, webSettings, adapter, instanceSettings, app) {
                    ...
                    for (var _id in values) {
                        ...
                        findState(_id, values.user, function (err, id, originId) {
                            if (err) {
                                ...
                                adapter.setForeignState(id, values[usedId], false, {user: values.user, checked:true}, function (err, id) {
                                    if (err) {
                                        doResponse(res, 'plain', 500, headers, 'error: ' + err, values.prettyPrint);
                                        cnt = 0;
                                    } else {
    ...
    this.restApi = function (req, res, isAuth, isChecked) {
        ...
        switch (command) {
                ...
                if (values.value === undefined && values.val === undefined) {
                    ...
                    findState(oId[0], values.user, function (err, id, originId) {
                        ...
                        if (id) {
                            ...
                            adapter.setForeignState(id, values.val, false, {user: values.user, checked:true}, function (err) {
                                if (err) {
                                    doResponse(res, 'plain', 500, headers, 'error: ' + err, values.prettyPrint);
                                    wait = 0;
                                } else
                            ...
                            if (err) {
                                ...
                                adapter.getForeignState(id, {user: values.user, checked:true}, function (err, state) {
                                    ...
                                    if (state) {
                                        ...
                                        adapter.setForeignState(id, state.val, false, {user: values.user, checked:true}, function (err) {
                                            if (err) {
                                                doResponse(res, 'plain', 500, headers, 'error: ' + err, values.prettyPrint);
                                                wait = 0;
                                            } else
            ...
            case 'getObjects':
            case 'objects':
                adapter.getForeignObjects(values.pattern || parts[2] || '*', values.type, {user: values.user, checked:true}, function (err, list) {
                    if (err) {
                        status = 500;
            ...
            case 'getStates':
            case 'states':
                adapter.getForeignStates(values.pattern || parts[2] || '*', {user: values.user, checked:true}, function (err, list) {
                    if (err) {
                        status = 500;
                        doResponse(res, responseType, status, headers, {error: JSON.stringify(err)}, values.prettyPrint);
                    } else {
