...
that.adapter.log.error('before findState');
        findState(idOrName, user, type, function (err, id, originId) {
that.adapter.log.error('after findState ' + err + ', id=' + id + '/' + originId);
            if (err) {
...
that.adapter.log.error('after getForeignState ' + err + ', obj=' + JSON.stringify(obj));
                    if (err || !obj) {
                        obj = undefined;
                    }
...
that.adapter.log.info('getPlainValue after getState: ' + err + ', ' + JSON.stringify(obj) + ', ' + id + ', ' + originId + ', '+response);
                        if (err) {
                            status = 500;
                            response = 'error: ' + err;
                            pcnt = 1;
